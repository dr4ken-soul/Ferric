...
/** Render the live Groq interaction capture and browser-local replay demonstration. */
export function LiveDemo() {
  ...
  const [message, setMessage] = useState('No interaction captured in this session.')
  const [replayText, setReplayText] = useState<string | null>(null)

  /** Send one prompt to the server-side Groq recorder. */
  async function recordInteraction() {
    setStatus('recording')
    setMessage('Calling Groq and building a Ferric cassette...')
    setReplayText(null)
    try {
      ...
      setStatus('idle')
      setMessage(`Captured ${payload.cassette.events.length} events in cassette ${payload.cassette.id.slice(0, 8)}.`)
    } catch (error) {
      setStatus('error')
    ...
    <section className="border-y border-[var(--border-default)] bg-[var(--bg-secondary)] px-4 py-24 md:px-8 md:py-32 lg:px-12" id="live-demo">
      <div className="mx-auto max-w-[1400px]">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--accent)]">Hosted interaction demo</p>
            <h2 className="mt-5 max-w-[16ch] font-display text-[clamp(2.25rem,5vw,4.5rem)] font-extrabold uppercase leading-[0.9] text-[var(--text-primary)]">Capture an interaction. Replay it here.</h2>
          </div>
          <p className="max-w-[48ch] font-body text-[15px] leading-[1.6] text-[var(--text-secondary)]">This is not a video recorder. It captures one Groq request and response as a Ferric cassette, then replays that stored response in your browser without calling the model again.</p>
        </div>
          ...
          <div className="p-6 md:p-8">
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <button className="inline-flex items-center gap-2.5 bg-[var(--accent)] px-5 py-3 font-mono text-[11px] font-medium uppercase tracking-[0.14em] text-[var(--bg-primary)] transition-colors duration-[240ms] hover:bg-[var(--accent-hover)] disabled:cursor-wait disabled:opacity-60" disabled={status === 'recording' || !prompt.trim()} onClick={recordInteraction} type="button">
                {status === 'recording' ? 'Capturing' : 'Capture with Groq'} <Icon className="h-3.5 w-3.5" name="record" />
              </button>
              <button className="inline-flex items-center gap-2.5 border border-[var(--border-default)] px-5 py-3 font-mono text-[11px] uppercase tracking-[0.14em] text-[var(--text-secondary)] transition-colors duration-[240ms] hover:border-[var(--accent)] hover:text-[var(--text-primary)] disabled:cursor-not-allowed disabled:opacity-40" disabled={!cassette || status === 'recording'} onClick={replayInteraction} type="button">
                Replay captured response <Icon className="h-3.5 w-3.5" name="play" />
              </button>
            </div>
          ...
          <div className="border-t border-[var(--border-default)] bg-[var(--bg-primary)] p-6 md:p-8 lg:border-t-0">
            {cassette ? (
              <>
                ...
                <div className="mt-5 flex justify-between border-t border-[var(--border-subtle)] pt-4 font-mono text-[10px] text-[var(--text-secondary)]"><span>{cassette.redactions.length} redactions</span><span>{status === 'replayed' ? 'replay: no network' : 'captured from Groq'}</span></div>
              </>
            ) : <div className="flex min-h-64 items-center justify-center border border-dashed border-[var(--border-default)] font-mono text-[11px] uppercase tracking-[0.16em] text-[var(--text-muted)]">Cassette appears after capture</div>}
          </div>
        </div>
      </div>
    </section>
